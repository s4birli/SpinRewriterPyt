...
from SpinRewriterPyt import exceptions as ex
import requests
import re
import json
import string
...
class Api(object):
    """A class representing the SpinRewriter API
    """
    API_URL = 'http://www.spinrewriter.com/action/api'
    """URL for invoking the API"""

    _list = ['api_quota', 'text_with_spintax',
             'unique_variation', 'unique_variation_from_spintax']
    ACTION = namedtuple('ACTION', _list)(*_list)
    """action parameter collection"""

    _list = ['low', 'medium', 'high']
    CONFIDENCE_LVL = namedtuple(
        'CONFIDENCE_LVL', ['low', 'medium', 'high'])(*_list)
    """onfidence_level parameter collection"""

    SPINTAX_FORMAT = namedtuple('SPINTAX_FORMAT', [
        'pipe_curly', 'tilde_curly', 'pipe_square', 'spin_tag'])(*['{|}', '{~}', '[|]', '[spin]'])
    """spintax_format parameter collection"""

    _list = ['email_address', 'api_key', 'action', 'text',
             'protected_terms', 'confidence_level', 'nested_spintax', 'spintax_format']
    REQ_P_NAMES = namedtuple('REQ_P_NAMES', _list)(*_list)
    """request parameters collection"""

    _list = ['status', 'response', 'api_requests_made',
             'api_requests_available', 'protected_terms',
             'confidence_level']
    RESP_P_NAMES = namedtuple('RESP_P_NAMES', _list)(*_list)
    """response fields collection"""

    _list = ['ok', 'error']
    STATUS = namedtuple('STATUS', ['ok', 'error'])(
        *map(lambda x: x.upper(), _list))
    """status response returned by API"""
    ...
    def _send_request(self, params):
        ...
        result = requests.post(self.API_URL, data=params)
